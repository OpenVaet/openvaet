...
while (<$in>) {
	...
	if ($dRNum == 1) {
		...
		$subjects{$subjectId}->{'deviations'}->{$deviationDate}->{'deviationCategory'} = $deviationCategory;
		$subjects{$subjectId}->{'deviations'}->{$deviationDate}->{'deviationTerm'}     = $deviationTerm;
