...
# Prints patients .CSV.
open my $out, '>:utf8', 'tasks/pfizer_trials/amyloidosis/adlb_2.csv';
say $out "subjectId;cohort;age;sex;aVisitNum;visitDatetime;param;avaLc;";
for my $subjectId (sort{$a <=> $b} keys %subjects) {
	my $cohort = $subjects{$subjectId}->{'cohort'} // die;
	my $sex    = $subjects{$subjectId}->{'sex'}    // die;
	my $age    = $subjects{$subjectId}->{'age'}    // die;
