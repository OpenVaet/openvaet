...
while (<$in>) {
	...
	if ($population eq 'Evaluable efficacy (7 days)') {
		if (
			...
			$otherExclusions{$subjectId}->{'arm'} = $arm;
			$otherExclusions{$subjectId}->{'populations'}->{$population} = $motive;
		} else {
			die "motive : [$motive]";
		}
...
# Prints patients JSON.
open my $out4, '>:utf8', "$outputFolder/pfizer_other_exclusions_subjects.json";
print $out4 encode_json\%otherExclusions;
close $out4;
