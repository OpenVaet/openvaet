...
for my $file (glob "public/pfizer_documents/pdf_to_html_files/*") {
	...
	for my $pageNum (sort{$a <=> $b} keys %pages) {
		...
		my %subjectIds = search_subject_ids(@divs);
	...
	for my $div (@divs) {
		...
		for my $word (@words) {
			if ($word =~ /\d\d\d\d\d\d\d\d/) {
				for my $subjectId (keys %sdSuppDs) {
					if ($word =~ /$subjectId/) {
						...
						$subjectIds{$topMargin}->{'subjectId'} = $subjectId;
						last;
					}
				}
			}
