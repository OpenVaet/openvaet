...
for my $subjectId (sort{$a <=> $b} keys %adsl) {
	...
	if ($mulenRfl ne 'Y' && $aai1effl eq 'Y') {
		$stats{'1_phase3'}->{'4_visit1TestingData'}->{'dose1TheoricalEfficacy'}++;
		$stats{'1_phase3'}->{'4_visit1TestingData'}->{'byArm'}->{$arm}->{'dose1TheoricalEfficacy'}++;
	}
	...
	if ($v1D1NBinding eq 'POS' || $v1D1CentralPCR eq 'POS') {
		...
		$stats{'1_phase3'}->{'6_visit1Testing'}->{'byArm'}->{$arm}->{'totalVisit1PositivePriorCutOff'}++;
	...
	unless ($dose2Date) {
		$stats{'1_phase3'}->{'8_dose2Administered'}->{'noDose2'}->{'total'}++;
		$stats{'1_phase3'}->{'8_dose2Administered'}->{'noDose2'}->{$arm}++;
		next;
	}
	if ($dose2Date > $cutoffCompdate) {
		$stats{'1_phase3'}->{'8_dose2Administered'}->{'dose2PostCutOff'}++;
		next;
	}
	...
	$stats{'1_phase3'}->{'8_dose2Administered'}->{'dose2PriorEfficacyCutOff'}++;
	$stats{'1_phase3'}->{'8_dose2Administered'}->{'byArm'}->{$arm}->{'priorEfficacyCutOff'}++;
	...
	if ($daysBetweenDoses < 19 || $daysBetweenDoses > 42) {
		$stats{'1_phase3'}->{'8_dose2Administered'}->{'outOf19To42DaysInterval'}++;
		next;
	}
	$stats{'1_phase3'}->{'8_dose2Administered'}->{'in19To42DaysInterval'}++;
	$stats{'1_phase3'}->{'8_dose2Administered'}->{'byArm'}->{$arm}->{'in19To42DaysInterval'}++;
	...
	if ($hasPositiveCentralPCR) {
		...
		if ($earliestCovid <= $dose2Date) {
			$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'covidPriorOrOnDose2'}++;
			$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'byArm'}->{$arm}->{'covidPriorOrOnDose2'}++;
			next;
		} else {
			...
			if ($daysFromCovidToDose2 < 7) {
				...
				# 	die;
				# }
				$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'covidPrior7DaysPostDose2'}++;
				$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'byArm'}->{$arm}->{'covidPrior7DaysPostDose2'}++;
				next;
			} else {
				$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'covidPositivePost7DaysPostDose2'}++;
				$stats{'1_phase3'}->{'9_dose2Efficacy'}->{'byArm'}->{$arm}->{'covidPositivePost7DaysPostDose2'}++;
				# next;
			}
		}
	}
	...
	if (keys %symptomsByVisit && $hasSymptoms) {
		$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'1_subjects'}->{'symptomatic'}++;
		$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'1_subjects'}->{'byArm'}->{$arm}->{'symptomatic'}++;
		for my $visitName (sort keys %symptomsByVisit) {
			...
			$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'2_visits'}->{'total'}++;
			$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'2_visits'}->{$arm}++;
			...
			if ($symptomsWithCentralPCR) {
				if ($symptomsWithPositiveCentralPCR == 1) {
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithPCR'}->{'positivePCR'}->{$arm}++;
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithPCR'}->{'positivePCR'}->{'total'}++;
				}
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithPCR'}->{'total'}++;
			} else {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithoutPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'3_central'}->{'symptomsWithoutPCR'}->{'total'}++;
			}
			if ($symptomsWithLocalPCR) {
				if ($symptomsWithPositiveLocalPCR == 1) {
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithPCR'}->{'positivePCR'}->{$arm}++;
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithPCR'}->{'positivePCR'}->{'total'}++;
				}
				...
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithPCR'}->{'total'}++;
			} else {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithoutPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'4_local'}->{'symptomsWithoutPCR'}->{'total'}++;
			}
			if ($symptomsWithCentralPCR || $symptomsWithLocalPCR) {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'5_localOrCentral'}->{'symptomsWithPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'5_localOrCentral'}->{'symptomsWithPCR'}->{'total'}++;
			} else {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'5_localOrCentral'}->{'symptomsWithoutPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'5_localOrCentral'}->{'symptomsWithoutPCR'}->{'total'}++;
			}
			if ($symptomsWithCentralPCR && $symptomsWithLocalPCR) {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'symptomsWithPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'symptomsWithPCR'}->{'total'}++;

				if ($symptomsWithPositiveLocalPCR == 0 && $symptomsWithPositiveCentralPCR == 1) {
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'negativeLocalPositiveCentral'}->{$arm}++;
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'negativeLocalPositiveCentral'}->{'total'}++;
				}
				if ($symptomsWithPositiveLocalPCR == 1 && $symptomsWithPositiveCentralPCR == 0) {
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'positiveLocalNegativeCentral'}->{$arm}++;
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'positiveLocalNegativeCentral'}->{'total'}++;
				}
				...
				if (
					...
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'noConflict'}->{$arm}++;
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'noConflict'}->{'total'}++;
				} else {
					if ($symptomsWithPositiveLocalPCR == 1) {
					...
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'conflict'}->{'byGroups'}->{$arm}->{'total'}++;
					for my $device (sort keys %localDevices) {
						$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'conflict'}->{'byDevices'}->{$device}++;
					}
					$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'conflict'}->{'total'}++;
				}
			} else {
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'symptomsWithoutPCR'}->{$arm}++;
				$stats{'1_phase3'}->{'10_eligibleWithSymptoms'}->{'6_localAndCentral'}->{'symptomsWithoutPCR'}->{'total'}++;
			}
		}
	}
}
