...
sub usa_death_stats {
	for my $subjectId (sort{$a <=> $b} keys %adsl) {
		...
		next if $randomizationDate > 20210313;
		$stats{'totalSubjects'}++;
		...
		if ($deathDatetime) {
			...
			$deathCompdate =~ s/\D//g;
			die if $deathCompdate && ($deathCompdate > 20210313);
			$stats{'byArm'}->{$arm}->{'totalDeaths'}++;
			# $stats{'byArmAndTotalDoses'}->{$arm}->{$arm}->{'totalDeaths'}++;
			$stats{'byArmAndAge'}->{$ageGroup}->{$arm}->{'totalDeaths'}++;
			$stats{'byArmAndAge'}->{$ageGroup}->{$arm}->{'subjects'}->{$subjectId} = $deathDatetime;
		}
		$stats{'byArmAndAge'}->{$ageGroup}->{'doePlacebo'}+= $doePlacebo;
		$stats{'byArmAndAge'}->{$ageGroup}->{'doeBNT162b2'}+= $doeBNT162b2;
...
sub print_synthesis {
	open my $out, '>:utf8', 'by_ages.csv';
	say $out "Age Group;BNT162b2;;;;Placebo;;;;";
	say $out "Age Group;Deaths;Days Of Exposure;Per 100 000 / Year;Expected;Deaths;Days Of Exposure;Per 100 000 / Year;Expected;";
	for my $ageGroup (sort keys %{$stats{'byArmAndAge'}}) {
		next unless exists $expectedMortality{$ageGroup};
		my $bNT162b2Deaths      = $stats{'byArmAndAge'}->{$ageGroup}->{'BNT162b2 Phase 2/3 (30 mcg)'}->{'totalDeaths'}   // 0;
		my $placeboDeaths       = $stats{'byArmAndAge'}->{$ageGroup}->{'Placebo'}->{'totalDeaths'}   // 0;
		my $doeBNT162b2         = $stats{'byArmAndAge'}->{$ageGroup}->{'doeBNT162b2'} // 0;
		my $doePlacebo          = $stats{'byArmAndAge'}->{$ageGroup}->{'doePlacebo'}  // 0;
		my $personsYearBNT162b2 = $doeBNT162b2 / 365;
		my $personsYearPlacebo  = $doePlacebo / 365;
		my $bNT162b2DeathsPer100000 = nearest(0.01, $bNT162b2Deaths * 100000 / $personsYearBNT162b2);
		my $placeboDeathsPer100000  = nearest(0.01, $placeboDeaths  * 100000 / $personsYearPlacebo);
		my $refDeathsPer100000 = nearest(0.01, ($expectedMortality{$ageGroup}->{'2020'} + $expectedMortality{$ageGroup}->{'2021'}) / 2);
		say $out "$ageGroup;$bNT162b2Deaths;$doeBNT162b2;$bNT162b2DeathsPer100000;$refDeathsPer100000;$placeboDeaths;$doePlacebo;$placeboDeathsPer100000;$refDeathsPer100000;";
	}
	close $out;
}