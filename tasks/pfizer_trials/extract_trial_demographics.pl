...
for my $patientId (sort keys %patients) {
	...
	my $pageNum       = $patients{$patientId}->{'pageNum'}       // die;
	my $entryNum      = $patients{$patientId}->{'entryNum'}      // die;
	my $screeningDate = $patients{$patientId}->{'screeningDate'} // die;
	say $out "pd-production-040122/125742_S1_M5_5351_c4591001-interim-mth6-demographics.pdf;$pageNum;$entryNum;$patientId;$sex;$ageYears;$screeningDate;$hasHIV;$isPhase1;";
			...
			if (($word =~ /C4591001/ || $word =~ /^\d\d\d\d$/ || $word =~ /\d\d\d\d\d\d\d\d/)) {
				if ($word =~ /C4591001/) {
					...
					if ($trialData && $siteData) {
						...
						$patientsIds{$entryNum}->{'patientId'}          = $patientId;
						$patientsIds{$entryNum}->{'patientIdTopMargin'} = $topMargin;
