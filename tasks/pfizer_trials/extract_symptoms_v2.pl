...
while (<$in>) {
	...
	if ($dRNum == 1) {
		...
		my $visitName = $values{'AVISIT'}  // die;
		# if ($visitName eq 'COVID_A') {
		# 	$visitName = 1;
		...
		if ($endDt) {
			...
			$endDatetime = time::timestamp_to_datetime($endDt);
		}
		...
		$subjects{$subjectId}->{'symptomsReports'}->{$adtDatetime}->{'visitName'} = $visitName;
		$subjects{$subjectId}->{'symptomsReports'}->{$adtDatetime}->{'endDatetime'} = $endDatetime;
		$subjects{$subjectId}->{'symptomsReports'}->{$adtDatetime}->{'symptoms'}->{$param} = $value;
		# die;
