...
# p%deviations;
my %stats = ();
for my $subjectId (sort{$a <=> $b} keys %screening) {
	...
	my (
		$screeningDate, $screeningDateOrigin, $isPhase1, $randomizationGroup,
		$age, $sexName, $trialSiteId
	);
	unless (exists $screening{$subjectId} && $screening{$subjectId}->{'screeningDate'}) {
		$stats{'0_preliminaryExclusions'}->{'noScreeningDate'}++;
		next;
	}
	$screeningDate = $screening{$subjectId}->{'screeningDate'} // die;
	unless ($screeningDate <= 20201114) {
		$stats{'0_preliminaryExclusions'}->{'screenedPostDate'}++;
	} else {
		...
		if ($isPhase1 && ($isPhase1 eq 'Yes')) {
			...
			$stats{'1_totalPhase2Screened'}->{'totalSubjects'}++;
			$randomizationGroup = $randomization{$subjectId}->{'randomizationGroup'} // 'NA';
			$randomizationGroup = 'BNT162b2' if $randomizationGroup =~ /BNT162b2/;
			$stats{'1_totalPhase2Screened'}->{$randomizationGroup}++;
			($trialSiteId) = $subjectId =~ /(....)..../;
			$trialSiteId = 1231 if $trialSiteId == 4444;
			$stats{'excludedBySites'}->{$trialSiteId}->{'totalSubjectsScreened'}++;
		}
	}
}
...
for my $subjectId (sort{$a <=> $b} keys %exclusions) {
	...
	$trialSiteId = 1231 if $trialSiteId == 4444;
	$stats{'excludedBySites'}->{$trialSiteId}->{'excluded'}++;
...
my %sitesStats = ();
open my $out, '>:utf8', 'other_exclusions_by_sites.csv';
say $out "Trial Site Id;Site Name;Latitude;Longitude;Postal Code;Investigator;Address;City;State;Country;Total Exclusions;Total Dose 1 Subjects;";
for my $trialSiteId (sort{$a <=> $b} keys %{$stats{'excludedBySites'}}) {
	my $totalExclusions = $stats{'excludedBySites'}->{$trialSiteId}->{'excluded'} // 0;
	my $totalSubjectsScreened = $stats{'excludedBySites'}->{$trialSiteId}->{'totalSubjectsScreened'} // die;
	my $siteName = $sites{$trialSiteId}->{'name'} // die "trialSiteId : $trialSiteId";
	...
	my $state = $sites{$trialSiteId}->{'state'} // '';
	my $country = $sites{$trialSiteId}->{'country'} // die;
	my $city = $sites{$trialSiteId}->{'city'} // die;
	say $out "$trialSiteId;$siteName;$latitude;$longitude;$postalCode;$investigator;$address;$city;$state;$country;$totalExclusions;$totalSubjectsScreened;";
	$sitesStats{$trialSiteId}->{'trialSiteId'} = $trialSiteId;
	$sitesStats{$trialSiteId}->{'siteName'} = $siteName;
	$sitesStats{$trialSiteId}->{'latitude'} = $latitude;
	$sitesStats{$trialSiteId}->{'longitude'} = $longitude;
	$sitesStats{$trialSiteId}->{'postalCode'} = $postalCode;
	$sitesStats{$trialSiteId}->{'investigator'} = $investigator;
	$sitesStats{$trialSiteId}->{'address'} = $address;
	$sitesStats{$trialSiteId}->{'city'} = $city;
	$sitesStats{$trialSiteId}->{'country'} = $country;
	$sitesStats{$trialSiteId}->{'state'} = $state;
	$sitesStats{$trialSiteId}->{'totalExclusions'} = $totalExclusions;
	$sitesStats{$trialSiteId}->{'totalSubjectsScreened'} = $totalSubjectsScreened;
}
close $out;

open my $out2, '>:utf8', 'exclusions_by_sites.json';
print $out2 encode_json\%sitesStats;
close $out2;
...
sub load_screening {
	...
	%screening = %$json;
	# p%screening;
	say "[$screeningFile] -> subjects : " . keys %screening;
}