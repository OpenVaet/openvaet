...
my $miniumVisits = 50;
my @labels  = ('Full Data', 'Pre-EUA', 'Post-EUA');
open my $out, '>:utf8', 'main_sites_local_vs_central_march_13.csv';
my $line = "Trial Site Id;Trial Site Name;Trial Site City;Trial Site PI;";
for my $label (@labels) {
	$line   .= "$label - Total Visits;$label - Chi;$label - p-value;";
	$line   .= "$label - bNT162b2 Visits;$label - bNT162b2 Central PCRs;$label - bNT162b2 Local PCRs;$label - bNT162b2 No Local PCR;$label - bNT162b2 Central Percent Of Visits;$label - bNT162b2 Local Percent Of Visits;";
	$line   .= "$label - placebo Visits;$label - placebo Central PCRs;$label - placebo Local PCRs;$label - placebo No Local PCR;$label - placebo Central Percent Of Visits;$label - placebo Local Percent Of Visits;";
}
say $out $line;
for my $trialSiteId (sort{$a <=> $b} keys %{$stats{'symptomsAnalysisBySites'}}) {
	my $hasData = 0;
	for my $label (@labels) {
		my $totalVisits = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'visits'}->{'total'} // next;
		if ($totalVisits < $miniumVisits) {
			$stats{'visitsStats'}->{"lessThan$miniumVisits"}++;
			next;
		} else {
			$stats{'visitsStats'}->{"moreThan$miniumVisits"}++;
			$stats{'visitsStats'}->{"moreThan$miniumVisits" . "Sites"}->{$trialSiteId}->{$label}++;
		}
		$hasData = 1;
		last;
	}
	...
	my $trialSitePI   = $sites{$trialSiteId}->{'investigator'} // die;
	say "*" x 50;
	say "*" x 50;
	say "trialSiteId                    : $trialSiteId";
	say "trialSiteName                  : $trialSiteName";
	say "trialSiteCity                  : $trialSiteCity";
	say "trialSitePI                    : $trialSitePI";
	my $line = "$trialSiteId;$trialSiteName;$trialSiteCity;$trialSitePI;";
	for my $label (@labels) {
		my $totalVisits = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'visits'}->{'total'} // die;
		my $currentMinimum = $miniumVisits;
		if ($label ne 'Full Data') {
			$currentMinimum = $currentMinimum  / 2;
		}
		if ($totalVisits < $currentMinimum) {
			$stats{'visitsStats'}->{"lessThan$currentMinimum"}++;
			$line .= ";;;;;;;;;;;;;;;";
			next;
		} else {
			$stats{'visitsStats'}->{"moreThan$currentMinimum"}++;
			$stats{'visitsStats'}->{"moreThan$currentMinimum" . "Sites"}->{$trialSiteId}->{$label}++;
		}

		# Fetching raw stats required to compute chi-square p-value.
		my $bNT162b2Visits                 = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'visits'}->{'BNT162b2'}                // die;
		my $placeboVisits                  = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'visits'}->{'Placebo'}                 // die;
		my $bNT162b2CentralSymptomsPCRs    = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'symptoms'}->{'central'}->{'BNT162b2'} // die;
		my $placeboCentralSymptomsPCRs     = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'symptoms'}->{'central'}->{'Placebo'}  // die;
		my $bNT162b2LocalSymptomsPCRs      = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'symptoms'}->{'local'}->{'BNT162b2'}   // 0;
		my $placeboLocalSymptomsPCRs       = $stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{$label}->{'symptoms'}->{'local'}->{'Placebo'}    // 0;
		my $bNT162b2CentralPercentOfVisits = nearest(0.01, $bNT162b2CentralSymptomsPCRs * 100 / $bNT162b2Visits);
		my $placeboCentralPercentOfVisits  = nearest(0.01, $placeboCentralSymptomsPCRs  * 100 / $placeboVisits);
		my $bNT162b2LocalPercentOfVisits   = nearest(0.01, $bNT162b2LocalSymptomsPCRs   * 100 / $bNT162b2Visits);
		my $placeboLocalPercentOfVisits    = nearest(0.01, $placeboLocalSymptomsPCRs    * 100 / $placeboVisits);
		my $bNT162b2NoLocalPcr             = $bNT162b2Visits - $bNT162b2LocalSymptomsPCRs;
		my $placeboNoLocalPcr              = $placeboVisits - $placeboLocalSymptomsPCRs;
		my $chi                            = chi_squared($bNT162b2LocalSymptomsPCRs, $bNT162b2NoLocalPcr, $placeboLocalSymptomsPCRs, $placeboNoLocalPcr);
		my $pValue                         = 1 - Math::CDF::pchisq($chi, $df);
		$line .= "$totalVisits;$chi;$pValue;";
		$line .= "$bNT162b2Visits;$bNT162b2CentralSymptomsPCRs;$bNT162b2LocalSymptomsPCRs;$bNT162b2NoLocalPcr;$bNT162b2CentralPercentOfVisits;$bNT162b2LocalPercentOfVisits;";
		$line .= "$placeboVisits;$placeboCentralSymptomsPCRs;$placeboLocalSymptomsPCRs;$placeboNoLocalPcr;$placeboCentralPercentOfVisits;$placeboLocalPercentOfVisits;";
		say "*" x 50;
		say "label                          : $label";
		say "totalVisits                    : $totalVisits";
		say "*" x 50;
		say "bNT162b2Visits                 : $bNT162b2Visits";
		say "bNT162b2CentralSymptomsPCRs    : $bNT162b2CentralSymptomsPCRs";
		say "bNT162b2LocalSymptomsPCRs      : $bNT162b2LocalSymptomsPCRs";
		say "bNT162b2NoLocalPcr             : $bNT162b2NoLocalPcr";
		say "bNT162b2CentralPercentOfVisits : $bNT162b2CentralPercentOfVisits";
		say "bNT162b2LocalPercentOfVisits   : $bNT162b2LocalPercentOfVisits";
		say "*" x 50;
		say "placeboVisits                  : $placeboVisits";
		say "placeboCentralSymptomsPCRs     : $placeboCentralSymptomsPCRs";
		say "placeboLocalSymptomsPCRs       : $placeboLocalSymptomsPCRs";
		say "placeboNoLocalPcr              : $placeboNoLocalPcr";
		say "placeboCentralPercentOfVisits  : $placeboCentralPercentOfVisits";
		say "placeboLocalPercentOfVisits    : $placeboLocalPercentOfVisits";
		# p$stats{'symptomsAnalysisBySites'}->{$trialSiteId};
	}
	say $out $line;
	# die;
}
close $out;
delete $stats{'symptomsAnalysisBySites'};
delete $stats{'visitsStats'};

open my $out2, '>:utf8', 'local_vs_central_march_13.csv';
$line    = "Label;Total Visits;Chi;p-value;";
$line   .= "bNT162b2 Visits;bNT162b2 Central PCRs;bNT162b2 Local PCRs;bNT162b2 No Local PCR;bNT162b2 Central Percent Of Visits;bNT162b2 Local Percent Of Visits;";
$line   .= "placebo Visits;placebo Central PCRs;placebo Local PCRs;placebo No Local PCR;placebo Central Percent Of Visits;placebo Local Percent Of Visits;";
say $out2 $line;
for my $label (@labels) {
	my $totalVisits = $stats{'symptomsAnalysis'}->{$label}->{'visits'}->{'total'} // die;

	# Fetching raw stats required to compute chi-square p-value.
	my $bNT162b2Visits                 = $stats{'symptomsAnalysis'}->{$label}->{'visits'}->{'BNT162b2'}                // die;
	my $placeboVisits                  = $stats{'symptomsAnalysis'}->{$label}->{'visits'}->{'Placebo'}                 // die;
	my $bNT162b2CentralSymptomsPCRs    = $stats{'symptomsAnalysis'}->{$label}->{'symptoms'}->{'central'}->{'BNT162b2'} // die;
	my $placeboCentralSymptomsPCRs     = $stats{'symptomsAnalysis'}->{$label}->{'symptoms'}->{'central'}->{'Placebo'}  // die;
	my $bNT162b2LocalSymptomsPCRs      = $stats{'symptomsAnalysis'}->{$label}->{'symptoms'}->{'local'}->{'BNT162b2'}   // 0;
	my $placeboLocalSymptomsPCRs       = $stats{'symptomsAnalysis'}->{$label}->{'symptoms'}->{'local'}->{'Placebo'}    // 0;
	my $bNT162b2CentralPercentOfVisits = nearest(0.01, $bNT162b2CentralSymptomsPCRs * 100 / $bNT162b2Visits);
	my $placeboCentralPercentOfVisits  = nearest(0.01, $placeboCentralSymptomsPCRs  * 100 / $placeboVisits);
	my $bNT162b2LocalPercentOfVisits   = nearest(0.01, $bNT162b2LocalSymptomsPCRs   * 100 / $bNT162b2Visits);
	my $placeboLocalPercentOfVisits    = nearest(0.01, $placeboLocalSymptomsPCRs    * 100 / $placeboVisits);
	...
	my $chi                            = chi_squared($bNT162b2LocalSymptomsPCRs, $bNT162b2NoLocalPcr, $placeboLocalSymptomsPCRs, $placeboNoLocalPcr);
	my $pValue                         = 1 - Math::CDF::pchisq($chi, $df);
	my $line = "$label;$totalVisits;$chi;$pValue;";
	$line .= "$bNT162b2Visits;$bNT162b2CentralSymptomsPCRs;$bNT162b2LocalSymptomsPCRs;$bNT162b2NoLocalPcr;$bNT162b2CentralPercentOfVisits;$bNT162b2LocalPercentOfVisits;";
	$line .= "$placeboVisits;$placeboCentralSymptomsPCRs;$placeboLocalSymptomsPCRs;$placeboNoLocalPcr;$placeboCentralPercentOfVisits;$placeboLocalPercentOfVisits;";
	say "*" x 50;
	say "label                          : $label";
...
sub symptoms_positive_data {
	...
	for my $subjectId (sort{$a <=> $b} keys %symptoms) {
		...
		$trialSiteId = 1231 if $trialSiteId == 4444;
		...
		my ($hasSymptoms, %symptomsByVisit) = subject_symptoms_by_visits($subjectId, $unblindCompdate);
		next unless keys %symptomsByVisit && $hasSymptoms;

		$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'subjects'}->{'withSymptoms'}->{'total'}++;
		$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'subjects'}->{'withSymptoms'}->{$randomizationGroup}++;
		next if $randomizationGroup eq 'Unknown';

		# Reorganizing Central PCRs by dates.
		my ($hasPositiveCentralPCR,
			%centralPCRsByVisit)    = subject_central_pcrs_by_visits($subjectId, $unblindCompdate);

		# Reorganizing Local PCRs by dates.
		my ($hasPositiveLocalPCR,
			%localPCRsByVisit)      = subject_local_pcrs_by_visits($subjectId, $unblindCompdate);
		for my $visitName (sort keys %symptomsByVisit) {
			...
			$stats{'symptomsAnalysis'}->{'Full Data'}->{'visits'}->{'total'}++;
			$stats{'symptomsAnalysis'}->{'Full Data'}->{'visits'}->{$randomizationGroup}++;
			$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'visits'}->{'total'}++;
			$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'visits'}->{$randomizationGroup}++;
			my $symptomCompdate = $symptomDate;$symptomCompdate =~ s/\D//g;
			if ($symptomCompdate <= 20201114) {
				$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'visits'}->{'total'}++;
				$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'visits'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'visits'}->{'total'}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'visits'}->{$randomizationGroup}++;
			} else {
				$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'visits'}->{'total'}++;
				$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'visits'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'visits'}->{'total'}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'visits'}->{$randomizationGroup}++;
			}
			...
			my $centralVisitDate = $centralPCRsByVisit{$visitName}->{'visitDate'};
			my $centralPcrResult = $centralPCRsByVisit{$visitName}->{'pcrResult'};
			...
			my $localVisitDate       = $localPCRsByVisit{$visitName}->{'visitDate'};
			my $localPcrResult       = $localPCRsByVisit{$visitName}->{'pcrResult'};
			my $localSpDevId         = $localPCRsByVisit{$visitName}->{'spDevId'};
			my %localDevices         = ();
			...
			if ($symptomsWithCentralPCR) {
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'symptoms'}->{'central'}->{'total'}++;
				$stats{'symptomsAnalysis'}->{'Full Data'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysis'}->{'Full Data'}->{'symptoms'}->{'central'}->{'total'}++;
				if ($symptomCompdate <= 20201114) {
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'symptoms'}->{'central'}->{'total'}++;
					$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'symptoms'}->{'central'}->{'total'}++;
				} else {
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'symptoms'}->{'central'}->{'total'}++;
					$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'symptoms'}->{'central'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'symptoms'}->{'central'}->{'total'}++;
				}
			}
			if ($symptomsWithLocalPCR) {
				$stats{'symptomsAnalysis'}->{'Full Data'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysis'}->{'Full Data'}->{'symptoms'}->{'local'}->{'total'}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
				$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Full Data'}->{'symptoms'}->{'local'}->{'total'}++;
				if ($symptomCompdate <= 20201114) {
					$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysis'}->{'Pre-EUA'}->{'symptoms'}->{'local'}->{'total'}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Pre-EUA'}->{'symptoms'}->{'local'}->{'total'}++;
				} else {
					$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysis'}->{'Post-EUA'}->{'symptoms'}->{'local'}->{'total'}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'symptoms'}->{'local'}->{$randomizationGroup}++;
					$stats{'symptomsAnalysisBySites'}->{$trialSiteId}->{'Post-EUA'}->{'symptoms'}->{'local'}->{'total'}++;
				}
			}
		}
	}
...
sub subject_central_pcrs_by_visits {
	...
	for my $visitDate (sort keys %{$pcrRecords{$subjectId}->{'mbVisits'}}) {
		...
		$centralPCRsByVisit{$visitName}->{'visitDate'} = $visitDate;
		$centralPCRsByVisit{$visitName}->{'pcrResult'} = $pcrResult;
		if ($pcrResult eq 'POS') {
...
sub subject_local_pcrs_by_visits {
	...
	for my $visitDate (sort keys %{$pcrRecords{$subjectId}->{'mbVisits'}}) {
		...
		$localPCRsByVisit{$visitName}->{'visitDate'} = $visitDate;
		$localPCRsByVisit{$visitName}->{'pcrResult'} = $pcrResult;
		$localPCRsByVisit{$visitName}->{'spDevId'}   = $spDevId;
		if ($pcrResult eq 'POS') {
...
sub subject_symptoms_by_visits {
	...
	for my $symptomDatetime (sort keys %{$symptoms{$subjectId}->{'symptomsReports'}}) {
		...
		for my $symptomName (sort keys %{$symptoms{$subjectId}->{'symptomsReports'}->{$symptomDatetime}->{'symptoms'}}) {
			...
			$symptomsByVisit{$visitName}->{'symptoms'}->{$symptomName} = 1;
			$totalSymptoms++;
		}
		next unless $totalSymptoms;
		$hasSymptoms  = 1;
		$symptomsByVisit{$visitName}->{'onsetStartOffset'}    = $onsetStartOffset;
		$symptomsByVisit{$visitName}->{'formerSymptomDate'}   = $formerSymptomDate;
		$symptomsByVisit{$visitName}->{'symptomDate'}         = $symptomDate;
		$symptomsByVisit{$visitName}->{'totalSymptoms'}       = $totalSymptoms;
		$symptomsByVisit{$visitName}->{'endDatetime'}         = $endDatetime;
		$symptomsByVisit{$visitName}->{'hasOfficialSymptoms'} = $hasOfficialSymptoms;
	}
	# p%symptomsByVisit;
	# die;
	return (
