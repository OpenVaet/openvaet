...
open my $outReport, '>:utf8', 'daily_clout_output/match_hypertension_VAERSDATA_reports.csv';
...
open my $outStat, '>:utf8', 'daily_clout_output/match_hypertension_VAERSDATA_stats.csv';
	...
	while (<$dataIn>) {
		...
		if ($dRNum == 1) {
			...
			for my $symptomName (sort keys %{$reportsSymptoms{$vaersId}}) {
				...
				if ($symptomNormalized =~ /hypertension/) {
					$hasSymptoms = 1;
				}
