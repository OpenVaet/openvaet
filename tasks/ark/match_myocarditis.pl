...
open my $outReport, '>:utf8', 'ark_output/match_myocarditis_VAERSDATA_reports.csv';
...
open my $outStat, '>:utf8', 'ark_output/match_myocarditis_VAERSDATA_stats.csv';
	...
	while (<$dataIn>) {
		...
		if ($dRNum == 1) {
			...
			for my $symptomName (sort keys %{$reportsSymptoms{$vaersId}}) {
				...
				if ($symptomNormalized =~ /myocarditis/) {
					$hasSymptoms = 1;
				}
