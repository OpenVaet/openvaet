...
make_path('ark_output') unless (-d 'ark_output');
...
open my $outReport, '>:utf8', 'ark_output/equal_myocarditis_VAERSDATA_reports.csv';
...
open my $outStat, '>:utf8', 'ark_output/equal_myocarditis_VAERSDATA_stats.csv';
	...
	while (<$dataIn>) {
		...
		if ($dRNum == 1) {
			...
			for my $symptomName (sort keys %{$reportsSymptoms{$vaersId}}) {
				...
				if ($symptomNormalized eq 'myocarditis') {
					$hasSymptoms = 1;
				}
			...
			my ($hasCovid, $hasOther, $hasCovidAndOther) = (0, 0, 0);
			for my $vaccData (@{$reportsVaccines{$vaersId}->{'vaccines'}}) {
				my $dose          = %$vaccData{'dose'}        // die;
				my $vaccineName   = %$vaccData{'vaccineName'} // die;
				my $substanceCategory   = %$vaccData{'substanceCategory'};
				if ($substanceCategory && ($substanceCategory eq 'COVID-19')) {
					$hasCovid = 1;
				} else {
					$hasOther = 1;
				}
				$lastProductTaken = $vaccineName;
				$lastDoseTaken    = $dose;
				# push @{$o{'vaccines'}}, \%$vaccData;
			}
			die unless $lastProductTaken;
			my $chartCategory;
			if ($hasCovid == 1 && $hasOther == 1) {
				$hasCovidAndOther = 1;
				$chartCategory = 'COVID-19+OTHERS'
			} elsif ($hasCovid == 1 && $hasOther == 0) {
				$chartCategory = 'COVID-19'
			} else {
				$chartCategory = 'OTHERS'
			}
			...
			$stats{'All Files'}->{'byChartCategory'}->{$chartCategory}->{'total'}++;
			$stats{$fileShort}->{'byChartCategory'}->{$chartCategory}->{'total'}++;
			if ($lastProductTaken) {
