...
# Getting list of reports states by states if an updated is required (no update has been fully performed on current day).
...
sub search_reports_by_states {
	...
	# Listing states, ages & sexes filters.
	list_all_states();
	list_all_ages();
	list_all_sexes();
...
sub list_all_states {
	...
	for my $stateOption (@states) {
		...
		$state    =~ s/ //g;
		next if
			$state eq 'All Locations'                         ||
			$state eq 'The United States/Territories/Unknown' ||
			$state eq 'Unknown'                               ||
			$state eq 'Foreign';
		# say "$state - $value";
...
sub parse_vaers_reports {
	...
	my @vaersReports = $responseForm->find('th');
	my $parsed = 0;
	for my $varsReportData (@vaersReports) {
		...
		unless (exists $cdcReports{$varsReport}->{'cdcReportId'}) {
			my $sth = $dbh->prepare("INSERT INTO cdc_report (cdcStateId, internalId) VALUES ($cdcStateId, ?)");
			$sth->execute($varsReport) or die $sth->err();
			cdc_reports();
		}
